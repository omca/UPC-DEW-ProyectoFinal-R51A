...
class LocalsControllerTest < ActionController::TestCase
  ...
  test "should update local" do
    patch :update, id: @local, local: { address: @local.address, district_id: @local.district_id, maps_x: @local.maps_x, maps_y: @local.maps_y, name: @local.name, phone: @local.phone }
    assert_redirected_to local_path(assigns(:local))
  end
